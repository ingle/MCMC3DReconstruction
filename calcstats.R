...
for( nsli in c(4,6,12,16) )
{
    ...
    for( nset in 1:5 )
    {
        load(paste('processed_',nset,'_',nsli,'.RData',sep=''))
        frecons3d = frecons3d * 0.868^2
...
allstatslnb <- expand.grid( c(4,6,12,16), 1:5 )
colnames(allstatslnb) <- c("nsli", "nset")
allstatslnb$bkgmean   <- rep(0,times=20)
allstatslnb$bkgstdv   <- rep(0,times=20)
allstatslnb$incmean   <- rep(0,times=20)
allstatslnb$incstdv   <- rep(0,times=20)
allstatslnb$bkgsnr    <- rep(0,times=20)
allstatslnb$incsnr    <- rep(0,times=20)
allstatslnb$c         <- rep(0,times=20)
allstatslnb$cnr       <- rep(0,times=20)

for( idx in 1:nrow(allstatslnb) )
{
    #load(paste('processed_nnb_',allstatsnnb$nset[idx],'_',allstatsnnb$nsli[idx],'.RData',sep=''))
    lnbcsv = read.table(paste('processed_linearnb_',allstatslnb$nset[idx],'_',allstatslnb$nsli[idx],'.csv',sep=''),sep=' ', header=T)
...
for( nsli in c(4,6,12,16) )
{
    ...
    for( nset in 1:5 )
    {
        #load(paste('processed_linearnb_',nset,'_',nsli,'.RData',sep=''))
        lnbcsv = read.table(paste('processed_linearnb_',nset,'_',nsli,'.csv',sep=''), sep=' ', header=T)
        frecons3d = array(lnbcsv$f, c(100,100,90))    
        frecons3d[frecons3d<0] = 0
        frecons3d[frecons3d>4] = 4
        frecons3d = frecons3d * 0.868^2
