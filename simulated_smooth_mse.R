...
if (!file.exists('simulatedsmoothmseresults.RData'))
{
    ...
    for(snr in SNR)
    {
        for( p in P )
        {
            ...
            dat <- getSmoothEllipsoidSheafData(P=p)
    ...
    save.image("simulatedsmoothmseresults.RData")
    cat('results saved. done.\n')
} else
{
    cat('data file exists. loading from disk.\n')
    load('simulatedsmoothmseresults.RData')
    mse4lev <- mse
    ...
    {
      cat('here\n')
      dev.copy2eps(file='~/Insync/Papers Ultrasound/Full3D Reconstruction/full3dsmoothmse.eps')
    }
    ...
    if(saveplots)
    {
      dev.copy2eps(file='~/Insync/Papers Ultrasound/Full3D Reconstruction/nnbsmoothmse.eps')
    }
    ...
    if(saveplots)
    {
      dev.copy2eps(file='~/Insync/Papers Ultrasound/Full3D Reconstruction/nnbmrf_smooth_mse_db.eps')
    }
    if(saveplots)
    {
      graphics.off()
